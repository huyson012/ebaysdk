...
use \SONHH\eBaySDK\Types;
use \SONHH\eBaySDK\Exceptions;
use \SONHH\eBaySDK\JmesPath\Env;
use \SONHH\eBaySDK\JmesPath\JmesPathableObjectInterface;
...
class BaseType implements JmesPathableObjectInterface
{
     ...
     *
     * @throws \SONHH\eBaySDK\Exceptions\UnknownPropertyException If the property does not exist.
     * @throws \SONHH\eBaySDK\Exceptions\InvalidPropertyTypeException If the value is the wrong type for the property.
     */
     ...
     * @param mixed $value. The value to assign to the property.
     *
     * @throws \SONHH\eBaySDK\Exceptions\InvalidPropertyTypeException If trying to assign a non array type to an repeatable property.
     */
    private function setValue($class, $name, $value)
    {
        ...
        if (!$info['repeatable']) {
            ...
            if ('array' !== $actualType) {
                throw new Exceptions\InvalidPropertyTypeException($name, 'SONHH\eBaySDK\Types\RepeatableType', $actualType);
            } else {
    ...
    private static function ensurePropertyType($class, $name, $value)
    {
        ...
        foreach ($valid as $check) {
            if ($check !== 'any' && \SONHH\eBaySDK\checkPropertyType($check)) {
    ...
    {
        if (is_subclass_of($value, '\SONHH\eBaySDK\Types\BaseType', false)) {
