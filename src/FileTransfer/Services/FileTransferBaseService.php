 ...
 */
class FileTransferBaseService extends \SONHH\eBaySDK\Services\BaseService
{
    /**
    ...
    public static function getConfigDefinitions()
    {
        ...
        return $definitions + [
            'apiVersion' => [
                'valid' => ['string'],
                'default' => \SONHH\eBaySDK\FileTransfer\Services\FileTransferService::API_VERSION
            ],
            'authToken' => [
                'valid' => ['string'],
                'required' => true
    ...
    protected function callOperationAsync($name, \SONHH\eBaySDK\Types\BaseType $request, $responseClass)
    {
        ...
        if ($name === 'uploadFile' && $request->hasAttachment()) {
            ...
            if (!isset($request->fileAttachment)) {
                $request->fileAttachment = new \SONHH\eBaySDK\FileTransfer\Types\FileAttachment();
            }

            if (!isset($request->fileAttachment->Data)) {
                $request->fileAttachment->Data = new \SONHH\eBaySDK\FileTransfer\Types\Data([
                    'xopInclude' => new \SONHH\eBaySDK\FileTransfer\Types\XopInclude([
